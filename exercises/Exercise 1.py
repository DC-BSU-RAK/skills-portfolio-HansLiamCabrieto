...
def displayMenu():
    menu_window = tk.Tk()
    menu_window.title("Math Quiz")

    tk.Label(menu_window, text="Select Difficulty Level:", font=("Times New Roman", 14)).pack(pady=10)

    # Difficulty buttons
    tk.Button(menu_window, text="Easy Level", width=15, command=lambda: startQuiz(menu_window, "Easy")).pack(pady=5)
    tk.Button(menu_window, text="Medium Level", width=15, command=lambda: startQuiz(menu_window, "Medium")).pack(pady=5)
    tk.Button(menu_window, text="Hard Level", width=15, command=lambda: startQuiz(menu_window, "Hard")).pack(pady=5)

    menu_window.mainloop()


def randomInt(difficulty): # number generation based on difficulty level
    if difficulty == "Easy":
...
def decideOperation(): #addition or substraction randomization
    return random.choice(["+", "-"])


def isCorrect(user_answer, correct_answer): #checking if the answer is correct
    return user_answer == correct_answer


def startQuiz(menu_window, difficulty): #starting the quiz
    menu_window.destroy()
    score = 0
    attempts = 0
    total_questions = 10
